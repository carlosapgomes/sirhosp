"""Management command for admission extraction (thin CLI wrapper).

This command delegates the actual extraction orchestration to the
``run_admission_extraction`` service. The service returns a structured
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if result.success:
            ...
            self.stderr.write(
                f"Admission extraction failed: {result.error_message}"
            )
            sys.exit(1)
