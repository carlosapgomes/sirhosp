...
class Command(BaseCommand):
    help = "Extract admissions report from source system for a specific date."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Extracting admissions from {start_date} to {end_date}...")

        # Path to the extract_admissions.py script
        script_path = (
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.stdout.write("Running admission extraction...")
            try:
                ...
                self.stderr.write(f"Admission extraction timed out: {exc}")
                sys.exit(1)
                ...
                self.stderr.write(f"Admission extraction failed: {exc}")
                sys.exit(1)
            ...
            if not json_files:
                # Empty — no admissions found (success, nothing to persist)
                ...
                self.stdout.write(
                    self.style.SUCCESS("No admissions found for the period.")
                )
                return
            ...
            try:
                ...
                self.stderr.write(f"Admission processing failed: {exc}")
                sys.exit(1)
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Admission extraction complete. "
