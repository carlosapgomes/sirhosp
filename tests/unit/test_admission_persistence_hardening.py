"""Tests for admission persistence hardening — idempotency and empty-output safety.

Slice S4 requirements (tasks.md 4.1-4.2):
- Repeated admission extraction persistence must not duplicate individual records.
- Empty admission output must persist a successful zero-count result with no stale records.
"""
...
def _clean_db():
    ...
    DailyAdmissionCount.objects.all().delete()
...
class TestIdempotentReExecution:
    """Repeated calls to process_admissions with same data must not duplicate records."""
    ...
    def test_repeat_same_records_produces_same_count(self):
        ...
        result1 = process_admissions(SAMPLE_RECORDS, reference_date=REF_DATE)
        assert result1["total_records"] == 3

        # Verify first run state
        daily1 = DailyAdmissionCount.objects.get(date=REF_DATE)
        assert daily1.count == 3
        assert daily1.records.count() == 3

        # Second run with same records
        result2 = process_admissions(SAMPLE_RECORDS, reference_date=REF_DATE)
        assert result2["total_records"] == 3

        # Verify no duplication
        daily2 = DailyAdmissionCount.objects.get(date=REF_DATE)
    ...
    def test_repeat_with_different_records_replaces_old(self):
        ...
        # First run: 3 records
        process_admissions(SAMPLE_RECORDS, reference_date=REF_DATE)
        ...
        result = process_admissions(updated_records, reference_date=REF_DATE)
        assert result["total_records"] == 2

        # Verify old records replaced
        daily = DailyAdmissionCount.objects.get(date=REF_DATE)
    ...
    def test_repeat_empty_records_after_having_records_clears_them(self):
        ...
        # First run: populate
        process_admissions(SAMPLE_RECORDS, reference_date=REF_DATE)
        assert DailyAdmissionCount.objects.get(date=REF_DATE).records.count() == 3

        # Second run: empty
        result = process_admissions([], reference_date=REF_DATE)
    ...
    def test_different_dates_do_not_interfere(self):
        ...
        process_admissions(SAMPLE_RECORDS, reference_date=REF_DATE)
        process_admissions(SAMPLE_RECORDS, reference_date=ANOTHER_DATE)

        daily1 = DailyAdmissionCount.objects.get(date=REF_DATE)
        daily2 = DailyAdmissionCount.objects.get(date=ANOTHER_DATE)
        ...
        # Different DailyAdmissionCount instances
...
class TestEmptyOutputPersistence:
    """Empty admission output must produce zero-count, no stale records."""

    def test_empty_records_creates_zero_daily_count(self):
        """Calling process_admissions with empty list creates a zero-count entry."""
        result = process_admissions([], reference_date=REF_DATE)
    ...
    def test_empty_records_leaves_no_stale_records(self):
        """Calling process_admissions with empty list has no records."""
        process_admissions([], reference_date=REF_DATE)
    ...
    def test_different_date_empty_preserves_other_dates(self):
        ...
        process_admissions(SAMPLE_RECORDS, reference_date=REF_DATE)
        process_admissions([], reference_date=ANOTHER_DATE)
...
class TestTransactionSafety:
    """process_admissions must be safe under partial failure scenarios."""
    ...
    def test_atomicity_on_failure_midway(self):
        ...
        call_count = [0]
        real_create = AdmissionRecord.objects.create
        ...
        with patch.object(AdmissionRecord.objects, "create", _failing_create):
            with pytest.raises(RuntimeError):
                process_admissions(
                    [{"PRONTUARIO": "NEW1", "NOME": "New 1", "DATA INTERNACAO": "01/06/2026"},
                     {"PRONTUARIO": "NEW2", "NOME": "New 2", "DATA INTERNACAO": "01/06/2026"}],
                    reference_date=REF_DATE,
                )

        # Verify rollback: the original 3 records must still exist because
        # the transaction was rolled back entirely, restoring the DailyAdmissionCount
        # and its records to pre-call state.
        daily = DailyAdmissionCount.objects.get(date=REF_DATE)
        assert daily.pk == orig_pk, "DailyAdmissionCount should be the same instance"
