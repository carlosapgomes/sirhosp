"""Characterization tests for death extraction service.

Slice S5 (tasks.md 5.1):
- Add characterization tests for extract_deaths service execution with
...
def _make_death_json(records_count: int = 3):
    """Create synthetic death JSON data matching the automation output."""
    records = []
        ...
        records.append(
            {
                ...
                "NOME": f"Paciente {i}",
                "OBITO": f"{i+1:02d}/{i+1:02d}/2026",
                "DATA OBITO": f"{i+1:02d}/{i+1:02d}/2026",
            }
        )
...
def _write_json(tmpdir_path: Path, data: dict, filename: str | None = None):
    ...
    filepath = tmpdir_path / (filename or "obitos-2026-06-01.json")
...
def _mock_tempdir_and_glob(records_count: int = 3):
    ...
    death records and patches ``tempfile.TemporaryDirectory`` and
    ...
    data = _make_death_json(records_count)
    ...
    with patch("apps.deaths.services.tempfile.TemporaryDirectory") as m_tmp:
...
class TestSuccessfulDeathExtraction:
    ...
    def test_extracts_and_persists_records(self, mock_credentials, mock_subprocess_success):
        ...
        assert result.extraction_type == "death_extraction"
        ...
        assert run.intent == "death_extraction"

        stages = list(run.stage_metrics.all())
        stage_names = [s.stage_name for s in stages]
        assert "death_extraction" in stage_names
        assert "death_persistence" in stage_names
...
@pytest.mark.django_db
class TestFailedDeathExtraction:
    ...
    def test_unexpected_exception_during_persistence(
        ...
        with patch("apps.deaths.services.process_deaths") as mock_process:
        ...
        assert stages[0].stage_name == "death_extraction"
        assert stages[0].status == "succeeded"
        # Second stage (persistence) failed
        assert stages[1].stage_name == "death_persistence"
...
class TestIngestionRunObservability:
    ...
    def test_run_uses_proper_intent(self, mock_credentials, mock_subprocess_success):
        """The IngestionRun intent should be death_extraction."""
        with _mock_tempdir_and_glob(records_count=1):
            result = run_death_extraction(
                start_date="01/06/2026",
                end_date="01/06/2026",
