"""Characterization tests for official census extraction service.

Slice C2-S1 (tasks.md 1.1-1.3):
- Add characterization tests for official census service execution with
...
def mock_subprocess_success():
    ...
    with patch("apps.census.services.run_subprocess") as mock:
        proc = MagicMock()
        proc.returncode = 0
        proc.stdout = ""
        proc.stderr = ""
...
def _make_official_census_json(records_count: int = 3):
        ...
        records.append(
            {
                ...
                "TEMPO INT": "5",
                "QUARTO/LEITO": f"LEITO{i:02d}",
                "CID INT": f"A{i:02d}",
                "DESCRICAO": f"Diagnóstico {i}",
                "UNIDADE": "UTI",
                "AREA FUNCIONAL": "AF01",
                "SIGLA": f"SIG{i}",
                "ESPECIALIDADE": "CLI",
...
def _write_json(
    tmpdir_path: Path, data: dict, filename: str | None = None
):
...
class TestSuccessfulOfficialCensusExtraction:
    """Happy path: extraction succeeds with various record counts."""

    def test_extracts_and_persists_records(
        ...
        assert result.extraction_type == "official_census_extraction"
        ...
        assert run.intent == "official_census_extraction"

        stages = list(run.stage_metrics.all())
        stage_names = [s.stage_name for s in stages]
        assert "official_census_extraction" in stage_names
        assert "official_census_persistence" in stage_names

    def test_extracts_zero_records(
        self, mock_credentials, mock_subprocess_success
    ):
    ...
    def test_no_json_file_means_zero_records(
        self, mock_credentials, mock_subprocess_success
    ):
    ...
    def test_preserves_existing_cli_date_format(
        self, mock_credentials, mock_subprocess_success
    ):
...
@pytest.mark.django_db
class TestFailedOfficialCensusExtraction:
    ...
    def test_subprocess_non_zero_exit(
    ...
    def test_unexpected_exception_during_persistence(
        ...
        """If persistence raises, the service should capture as unexpected_exception."""
        with patch(
        ...
        assert stages[0].stage_name == "official_census_extraction"
        assert stages[0].status == "succeeded"
        # Second stage (persistence) failed
        assert stages[1].stage_name == "official_census_persistence"
        assert stages[1].status == "failed"
        assert "DB connection lost" in json.dumps(
            stages[1].details_json
        )
...
class TestIngestionRunObservability:
    ...
    def test_stage_metrics_recorded_for_success(
        ...
        assert len(stages) == 2
        assert (
            stages[0].stage_name == "official_census_extraction"
        )
        assert stages[0].status == "succeeded"
        assert (
            stages[1].stage_name == "official_census_persistence"
        )
    ...
    def test_run_uses_proper_intent(
        ...
        """The IngestionRun intent should be official_census_extraction."""
        with _mock_tempdir_and_glob(records_count=1):
            result = run_official_census_extraction(
                date="01/06/2026",
            )
