...
@pytest.mark.django_db
def test_worker_loop_closes_stale_connections_before_poll_and_after_failure():
    """Loop mode should discard stale DB connections around polling failures."""
    failing_qs = MagicMock()
    failing_qs.count.side_effect = OperationalError("the connection is closed")
    ...
    assert close_old_connections.call_count == 3
