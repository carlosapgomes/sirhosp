...
A **Busca Histórica** é o que chamamos de "Rede de Segurança": se o paciente está no banco local, você o exibe; se não está (mas existiu no EHR original), o seu sistema funciona como um *proxy* que vai lá buscar, indexar e salvar.
...
2.  **O Fluxo "On-Demand":**
    * Se o usuário clica no paciente que está com a fonte "EHR (Sync)", você pode explicar que o sistema abrirá uma barra de progresso rápida: *"Coletando dados históricos do sistema original... 40%"*. Assim que terminar, ele cai na **Página de Timeline** que já desenhamos.
