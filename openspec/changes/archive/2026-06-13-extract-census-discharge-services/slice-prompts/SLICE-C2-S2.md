# SLICE C2-S2 - Official census persistence hardening
...
Implement only Slice C2-S2. Do not start the next slice.
...
Harden official census persistence so repeated execution and empty output are
deterministic.

## Suggested scope

Add failing tests first. Cover repeated persistence for the same date,
zero-record output, and stale-row cleanup. Keep changes local to official census
persistence code.
...
- official census extraction service module
- `tests/unit/test_official_census_persistence_hardening.py`
- optional existing official census service tests
...
Create `/tmp/sirhosp-slice-C2-S2-report.md` with:
