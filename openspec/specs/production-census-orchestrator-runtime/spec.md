...
Define the production runtime requirements for the adaptive census orchestrator
as a dedicated Docker Compose service with bounded volatile storage, browser
shared memory, log rotation, systemd management and operator validation/rollback
guidance, isolating the continuous census loop from the portal `web` service.
