# fullsync-failure-exhaustion-fix Specification

## Purpose

Fixes the full-sync failure exhaustion proven by ADR-0008: deterministic
payload failures fail fast without burning attempts, evolution windows get
a volume-scaled bounded budget, and the payload validations are pinned by a
characterization suite with proven sensitivity.
